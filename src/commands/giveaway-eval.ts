...
export default {
  ...
  async execute(interaction: CommandInteraction): Promise<void> {
    ...
    const giveaway_obj = await db.getAsync(
      `SELECT message_id, prize, timestamp, status, organizer_id FROM giveaways WHERE message_id = ? AND organizer_id = ?`,
      [ga_id, dc_id]
    )
    if (!giveaway_obj || giveaway_obj.status !== GIVEAWAY_STATUS.OPENED) {
      await interaction.reply({
        content: 'ERROR. Dir fehlt die Berechtigung oder das Giveaway existiert nicht. Wer weiß?',
        ephemeral: true
      })
      return
    }
    ...
    await interaction.reply({
      embeds: [new EmbedBuilder().setColor(Colors.Green).setTitle('Giveaway wurde beendet')],
      ephemeral: true
    })
  }
}
